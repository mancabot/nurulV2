...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color(' silahkan scann QR nya by NurulNih'))
	})
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Akhirnya beban group berkurang 1,bye bye🏃 @${num.split('@')[0]}jasa mu akan di kubur dalam² katanya:V`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
...
case 'timer':
		...
		client.sendMessage(from, bahasa(prefix, sender), text, {quoted: mek})
			...
			mentions(`Cieee, jadi admin🥳🥳🥳: @${mentioned[0].split('@')[0]}`, mentioned, true)
			client.groupMakeAdmin(from, mentioned)
		    }
		    break
				  case 'wa.me':
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
               ...
               client.sendMessage(from, 'tuh owner Manca-Bot >_<, jangan spam,vc,von, diblok nangis nanti',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                client.updatePresence(from, Presence.composing) 
                if (!isUser) return reply(mess.only.daftarB)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				if (!isQuotedImage) return reply(`Envie fotos com legendas ${prefix}setbotpp ou tags de imagem que já foram enviadas`)
					...
					reply('makasih foto profil nya NurulCans😗')
					break
				case 'bc':
					client.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `*「 MANCA-BOT BROADCAST 」*\n\n${body.slice(4)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of groupMembers) {
							client.sendMessage(_.jid, buff, image, {caption: `*「 MANCA-BOT BC GROUP 」*\n*Group* : ${groupName}\n\n${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of groupMembers) {
							sendMess(_.jid, `*「 MANCA-BOT BC GROUP 」*\n*Group* : ${groupName}\n\n${body.slice(6)}`)
						}
				...
				case 'alay':
					...
					client.groupLeave (from) 
                     ...
                     setTimeout( () => {
					client.updatePresence(from, Presence.composing) 
					client.sendMessage(from, 'Sampai jumpa semoga tenang di alam sana👋, Katanya:V', text) // ur cods
					}, 0)
                     break
			...
			case 'grouptutup':
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('❌ teu bisa omm ❌')
						buffer = fs.readFileSync(ran)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'DI SURUH² MAGER KAK'})
						fs.unlinkSync(ran)
					})
					break
                	case 'tomp3':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('❌ yah teu bisa')
						buffer = fs.readFileSync(ran)
